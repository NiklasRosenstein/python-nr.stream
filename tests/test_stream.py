...
def test_groupby():
    ...
    by_country = Stream(companies).groupby(key=lambda x: x["country"]).map(lambda x: (x[0], list(x[1]))).collect()
...
def test_groupby_2_with_types() -> None:
    ...
    result: Stream[t.Tuple[str, t.List[t.Tuple[str, str, str]]]] = (
        Stream(entries).sortby(lambda x: x[1]).groupby(lambda x: x[1], lambda it: list(it))
    )
    assert list(result) == [("general", [entries[0], entries[2]]), ("packaging", [entries[1]])]
